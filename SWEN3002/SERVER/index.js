...
const getCombinedPublicKey = function(groupId){
    return new Promise(function(resolve, reject){
        connection.query("SELECT publicKey FROM participants WHERE gid = '"+groupId+"';", function(error, results, fields){
            //BIG INT WARNING
            var combinedPublicKey = bigInt("1");
            for(var x = 0; x < results.length; x++){
                combinedPublicKey.multiply(results[x]["publicKey"]);
            }
            resolve(combinedPublicKey.toString());
        });
    });
}
...
router.post('/newgroup', async function(req, res){
    ...
    if(sha256(passphrase) == config.sha256Password){
        connection.query("INSERT INTO groups (joinKey) VALUES ('"+joinKey+"');", function(error, results, fields){
            connection.query("INSERT INTO participants (gid, username, publicKey) VALUES ('"+results["insertId"]+"', '"+username+"', '"+publicKey+"');", function(error, results, fields){
                res.send(joinKey);
            });
        });
    }
...
router.post('/joingroup', async function(req, res){
    const username = req.body.username;
    const publicKey = req.body.publicKey;
    const joinKey = req.body.joinKey;
...
router.post('/message', async function(req, res){
    ...
    const hashedMessage = req.body.hashedMessage;
    const r = req.body.r;
    const groupId = await getGroupIdFromJoinKey(joinKey);
    const participantId = await getParticipantIdFromGroupId(groupId, username);
    if(participantId != null){
        connection.query("INSERT INTO messages (pid, message, hashedMessage, r, signature) VALUES ('"+participantId+"', '"+message+"', '"+hashedMessage+"', '"+r+"', '"+signature+"');", function(error, results, fields){
            if(error)
                res.send("false");
            else{
                connection.query("SELECT * FROM participants WHERE gid = '"+groupId+"';", function(error, results, fields){
                    for(var x = 0; x < results.length; x++){
                        connection.query("INSERT INTO recipients (mid, pid) VALUES ('"+results["insertId"]+"', '"+results[x]["pid"]+"');", function(error, results, fields){
                            res.send("true");
                        });
                    }
                });
...
router.get('/messages', async function(req, res){
    ...
    try{
        ...
        connection.query("SELECT ts FROM participants WHERE username = '"+username+"' AND gid = '"+groupId+"';", function(error, results, fields){
...
router.get('/participants', async function(req, res){
    ...
    if(groupId == null)
        ...
        connection.query("SELECT * FROM participants WHERE gid = '"+groupId+"';", function(error, results, fields){
            for(var x = 0; x < results.length; x++){
                participants[results[x]["username"]] = results[x]["publicKey"];
            }
...
router.get('/anynewmessages', function(req, res){
    ...
    const groupId = getGroupIdFromJoinKey(joinKey);
    connection.query("SELECT ts FROM participants WHERE gid = '"+groupId+"' AND username = '"+username+"';", function(req, res){
        const userJoinTs = results[0]["ts"];
        const oldMessagesSqlArray = jsArrayToSqlArray(oldMessages);
        connection.query("SELECT * FROM messages JOIN participants ON messages.pid = participants.pid JOIN groups ON groups.gid = participants.pid WHERE groups.gid = '"+groupId+"' AND messages.mid NOT IN "+oldMessagesSqlArray+" AND messages.ts > '"+userJoinTs+"' AND  participants.leftChat = 0 GROUP BY messages.mid", function(error, results, fields){
            if(oldMessages.length < results.length){
                res.send("true");
            }
            else{
                res.send("false");
...
router.post('/connect', function (req, res) {
    const connectionPassword = sha256(req.body.connectionPassword);
    if(connectionPassword == config.sha256Password){
