...
const newMessage = function(senderIP, username, message, timestamp){
    return {
        "sender": senderIP,
        "username": username,
        "message": message,
        "ts": timestamp,
        "recipients": JSON.parse(JSON.stringify(participants))
    }
}
...
router.get('/connect', function (req, res) {
    ...
    var profilePic = req.query.profilePic;
    var users = {};
    if(profilePic == null)
...
router.get('/getmessages', function(req, res){
    const ip = req.connection.remoteAddress;
    const username = req.query.username;
    const oldMessages = JSON.parse(req.query.oldMessages);
    var newMessages = [];
    for(var checksum in messages){
        if(oldMessages[checksum] == null){
            if(messages[checksum]["recipients"][ip] != null && participants[ip]["username"] == username)
...
router.post('/message', function (req, res) {
    try{
        ...
        const checksum = md5(senderIP+username+message+ts);
        messages[checksum] = newMessage(senderIP, username, message, ts);
