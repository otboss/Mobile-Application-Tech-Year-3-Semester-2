...
class DatabaseManager {
  ...
  ///Columns Are: cid, username, profilePic, ts
  final String chatsTable = "chats";
  ///Columns Are: mid, cid, username, ip, msg, inbound, ts
  ...
  Future<bool> saveMessage(String message) async{
    var client = await getDatabase();
    try{
      if(await isPreviousChat() == false)
        client.rawInsert("INSERT INTO $chatsTable (username, profilePic) VALUES ('$peerUsername', '$peerProfilePic')");
      await client.rawInsert("INSERT INTO $messagesTable (username, ip, msg, inbound) VALUES ('$peerUsername', '$peerIpAddress', '$message', '1')");
      List query = await client.rawQuery("SELECT MAX(mid) FROM $messagesTable WHERE username = $peerUsername AND ip = $peerIpAddress");
    ...
    try{
      if(!isGroup){
        if(loadMore){
          ...
          results = await client.rawQuery("SELECT * FROM $messagesTable WHERE ip = '$ipAddress' AND username = '$username' ORDER BY ts DESC LIMIT "+negatePostsArray.length.toString());
          results = List.from(results)..addAll(
            await client.rawQuery("SELECT * FROM $messagesTable WHERE ip = '$ipAddress' AND username = '$username' WHERE mid NOT IN $negation ORDER BY ts DESC LIMIT $limitPerMessagesFetchFromDatabase")
          );
        }
        else{
          results = await client.rawQuery("SELECT * FROM $messagesTable WHERE ip = '$ipAddress' AND username = '$username' ORDER BY ts DESC LIMIT $limitPerMessagesFetchFromDatabase");
        }
      }
    }
    catch(err){
  ...
  Future<List> getPastConversations(List chatsToNegate, bool loadMore) async{
    var client = await getDatabase();
    List results = [];
    try{
      if(loadMore){
        String negation = listToSqlArray(chatsToNegate);
        results = await client.rawQuery("SELECT * FROM $chatsTable ORDER BY ts DESC LIMIT "+chatsToNegate.length.toString());
        results = List.from(results)..addAll(
          await client.rawQuery("SELECT * FROM $chatsTable WHERE cid NOT IN $negation ORDER BY ts DESC LIMIT $limitPerChatsFetchFromDatabase")
        );        
      }
      else  
        results = await client.rawQuery("SELECT * FROM $chatsTable ORDER BY ts DESC LIMIT $limitPerChatsFetchFromDatabase");
    }
    catch(err){
      print(err);
    }
